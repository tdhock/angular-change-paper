...
bkp <- function(Linear_diff, Constant_diff, param){
  data.table(Linear_diff, Constant_diff, param)
}
...
data.vec <- c(270,180,200)
data.vec <- c(3,103,230,250,200,190)
loss.dt.list <- list()
diff.dt.list <- list()
ptr.dt.list <- list()
...
for(data.i in seq_along(data.vec)){
  ...
  diff.dt.list[[data.i]] <- data.table(data.i, current.diffs)
  ...
  moves <- 0
  ptr.dt.list[[paste(data.i, "before_min")]] <- data.table(
    data.i, Pointer="before", param=pointer_param, moves,
  ...
  after_min <- data.table(
    data.i, Pointer="after", param=pointer_param, moves,
    loss=pointer_param*pointer_Linear+pointer_Constant)
  ptr.dt.list[[paste(data.i, "after_min")]] <- after_min
  ptr.dt.list[[paste(data.i, "after_max")]] <- data.table(
    after_min
  )[, `:=`(
    Pointer="after_max",
  ...
  loss.dt.list[[data.i]] <- grid.dt[, data.table(
    data.i,
    data.value,
    param=degrees,
...
gg <- ggplot()+
  ...
  geom_line(aes(
    param, cum.loss),
    data=loss.dt)+
  facet_wrap("data.i", scales="free", labeller=label_both)+
  geom_point(aes(
    param, loss, color=Pointer),
    data=ptr.dt)+
  scale_x_continuous(
    "parameter",
  ...
  scale_y_continuous(
    "Geodesic loss")
png("figure-pointer-moves.png", width=7, height=4, units="in", res=200)
print(gg)
dev.off()
